...
<h3>Saldo inicial periodo contable: <?=$per->anio." - ".$per->MesToStr($per->mes);?></h1>

<form>
	<table class="noborder" style="width: 100%">
		<!--<tr class="liste_titre">
			<td colspan="2" style="text-align: right">
				<a href="mayor_print.php?a=<?=$anio;?>&m=<?=$mes;?>" target="popup">
					Descargar
				</a>
			</td>
		</tr>-->
...
<?php
		...
		while ($ok == 1) {
			...
			if (!($sat->codagr == "100" || $sat->codagr == "200" || $sat->codagr == "300")) {
				$var = !$var;
				$pol->getSumDebeHaber($ctas->cta, $anio, $mes);
...
<?php 
								$arr = $pol->fetch_by_cuenta2($ctas->cta, $anio, $mes);
								$ad = array();
								$ah = array();
								if (is_array($arr)) {
?>
...
<?php 
										if ($i < sizeof($ad)) {
											...
											$debe_total+=$ad[$i]["debe"];
?>
											<td style="width: 25%">
												<a href="libro_diario.php?a=<?=$ad[$i]["anio"];?>&m=<?=$ad[$i]["mes"];?>&id=<?=$ad[$i]["pid"];?>" >
													<img src='<?="../images/lupa.png";?>' height='11px' width='11px'> <?php print $as." "; print $ad[$i]["pid"] - $consecutivo + 1;?>
												</a>
											</td>
...
<?php 
										...
										if ($i < sizeof($ah)) { 
											...
											$as="Poliza ";
											$as.=$pol2->Get_folio_poliza();
											$haber_total+=$ah[$i]["haber"];
?>
											<td style="width: 25%">
												<a href="libro_diario.php?a=<?=$ah[$i]["anio"];?>&m=<?=$ah[$i]["mes"];?>&id=<?=$ah[$i]["pid"];?>" >
													<img src='<?="../images/lupa.png";?>' height='11px' width='11px'> <?php print $as." "; print $ah[$i]["pid"] - $consecutivo + 1;?>
												</a>
											</td>
											<td style="width: 25%; text-align: right;">

...
dol_fiche_head($head, '4', 'Configuración', 0, ''); */
...
<form method="post" action="cuentas.php?mod=2&action=save">
<table class="noborder" style="width: 100%">
	<tbody>
		<tr class="liste_titre">
			<th style="width: 40%">Definicion de la Cuenta</th>
			<th>Cod. Agr. Relacionado<br>en Cat. Principal</th>
			<th>Numero de Cuenta<br>en Cat. del Cliente</th>
		</tr>
...
if ($action == "save") {
	if ($_POST) {
		foreach($_POST as $key => $value) {
			//print "<br>POST - key=".$key." value=".$value;
			if($rel->fetch_by_code($key)) {
				//print "<br>sat_codagr=".$rel->sat_codagr;
				if ($value == $rel->fk_cat_cta ) {
				} else {
					$rel->fk_cat_cta = $value;
					$rel->update_fk_cat_cta();
					$recargar = 1;
				}
			}
		}
...
$arr_cat = $cat->fetch_array();

/* foreach($arr_cat as $j => $ac) {
	print_r($ac);
	print "<br>";
} */
...
<?php 
				foreach($arr_cat as $j => $ac) {
					?><option value="<?=$ac->rowid;?>" <?=($ac->rowid == $a->fk_cat_cta) ? 'selected="selected"' : ''; ?>><?=$ac->cta." - ".$ac->descta;?></option><?php 
				}
?>
	       	</select>
	    </td>
...
if (!$config->HayRegistros_CatCtas() > 0){
	$msg = "Favor de realizar la importacion de su catalogo de cuentas o la creacion de su catalogo de cuentas en el Sistema.";
}

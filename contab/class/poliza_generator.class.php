...
class PolizaGenerator extends CommonObject 
{
	...
	function Crear_Polizas_Proveedores($user) {
		...
		if ($this->facid > 0 && $this->tipo_fac == $this::FACTURA_TIPO_PROVEEDOR) {
			...
			$sql .= " WHERE f.entity = ".$conf->entity." AND cp.rowid is null AND f.rowid = ".$this->facid;  // AND f.paye = 1 AND f.fk_statut = 2 
			$sql .= " ORDER BY f.ref "; */
			$sql="SELECT f.rowid, s.nom, f.ref, f.datef, f.type, f.fk_cond_reglement, b.dateo, pf.amount, pa.code, pa.libelle, pai.rowid as paimid
			 FROM ".MAIN_DB_PREFIX."facture_fourn as f
			     ...
			     LEFT JOIN (Select a.*,haber From ".MAIN_DB_PREFIX."contab_polizas a,".MAIN_DB_PREFIX."contab_polizasdet
			          		 Where societe_type = 2  AND (tipo_pol='E' || tipo_pol='C') AND a.rowid=fk_poliza AND haber!=0 )
							as cp ON f.rowid = cp.fk_facture AND cp.haber=pf.amount
			WHERE f.entity = ".$conf->entity." AND cp.rowid is null AND (f.fk_statut=1 || f.fk_statut=2) AND f.rowid = ".$this->facid;
			$sql .= " ORDER BY f.ref ";
					...
					if ($obj->type == $fac::TYPE_STANDARD) {
						...
						if ($cond_pago == $payment::PAGO_AL_CONTADO) {
							...
							while ($obj) {
								...
								$pol->Proveedor_Pago_Factura2($obj->paimid, $user, $conf, 0);
	...
	function Simular_Crear_Polizas_Proveedores($user) {
		global $conf;
					...
					if ($obj->type == $fac::TYPE_STANDARD) {
						...
						if ($cond_pago == $payment::PAGO_AL_CONTADO) {
							...
							while ($obj) {
								dol_syslog("Se genera la póliza por el pago al contado por el monto de esta transacción: paimid=".$obj->paimid);
								//$pol->Proveedor_Pago_Factura2($obj->paimid, $user, $conf, 0);
								$pol->Simular_Proveedor_Pago_Factura2($obj->paimid, $user, $conf, 0);
								$obj = $this->db->fetch_object($res);
							}
						} else if ($cond_pago == $payment::PAGO_A_CREDITO) {
							dol_syslog("Es una poliza de Proveedor, Compra a Crédito - facid=".$obj->rowid);
							...
							//como es a credito, primero se guarda la poliza por la compra a credito.
							dol_syslog("Se genera la póliza por compra a crédito");
							$pol->Simular_Proveedor_Compra_a_Credito2($obj->rowid, $user, $conf);
							//$pol->Proveedor_Compra_a_Credito2($obj->rowid, $user, $conf);
							dol_syslog("Se terminó de generar la póliza por la compra a crédito");
							//Ahora se genera la poliza por el pago de la compra a credito.
							//Puede haber varios pagos, así que hay que ver esto.
	...
	function Crear_Polizas_Clientes($user) {
		...
		WHERE f.entity = ".$conf->entity." AND cp.rowid is NULL AND fk_cond_reglement!=0 AND f.rowid = ".$this->facid." AND (f.fk_statut=1 || f.fk_statut=2)  ORDER BY f.facnumber";
	
			dol_syslog("Se muestran las facturas a Clientes sin pólizas en base al sql=$sql");
			if ($res = $this->db->query($sql)) {
				dol_syslog("Entre 1");
				if ($obj = $this->db->fetch_object($res)) {
					dol_syslog("Entre 2");
	...
	function Simulacion_Polizas_Clientes($user) {
		global $conf;

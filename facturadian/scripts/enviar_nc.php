...
fac.fk_facture_source AS rowidreferencia,
RIGHT(extra.invoicetypecode,1) AS invoicetypecode,
'00' AS PaymentMeansID,
...
depa.nom AS ReceptorAddressCountrySubentity,
...
CASE
	WHEN fac.total < 0 THEN TRUNCATE((fac.total * -1), 2)
	ELSE TRUNCATE(fac.total, 2)
END AS LineExtensionAmount,
CASE
	WHEN fac.tva < 0 THEN TRUNCATE((fac.tva * -1), 2)
	ELSE TRUNCATE(fac.tva, 2)
END AS tva,
CASE
	WHEN fac.total_ttc < 0 THEN TRUNCATE((fac.total_ttc * -1), 2)
	ELSE TRUNCATE(fac.total_ttc, 2)
...
depa.nom AS ReceptorDepartamento
...
INNER JOIN ".PREFIJO."_c_departements AS depa
ON soc.fk_departement = depa.rowid
INNER JOIN ".PREFIJO."_facture_extrafields AS extra
ON fac.rowid = extra.fk_object
INNER JOIN ".PREFIJO."_c_paiement AS paiement
ON fac.fk_mode_reglement = paiement.id
WHERE NOT extra.envio AND fac.type = '2' AND LEFT(fac.ref,1) <> '(' AND UPPER(LEFT(fac.ref,1)) <> 'U' AND fac.ref LIKE '".DOCUMENT."%' ORDER BY fac.ref
LIMIT ".LIMITE;
...
while($row = $result->fetch_array(MYSQLI_ASSOC)) {
	...
	$DocumentTypeCode = "NC";
	// hallar datos de referencia para la Nota
	$cadenaRef = "
		SELECT fac.ref AS ref,
		fac.datef AS datef,
		extra.cufe AS cufe
		FROM ".PREFIJO."_facture AS fac
		INNER JOIN ".PREFIJO."_facture_extrafields AS extra
		ON fac.rowid = extra.fk_object
		WHERE fac.rowid = '{$row['rowidreferencia']}' ";
	$resultRef = $dolibarr->leerdatos($cadenaRef);
	$invoiceRef = explode("-",$resultRef['ref']);
	
	$IDInvoiceDocumentReference = $invoiceRef[0].$invoiceRef[1];
	$IDInvoiceDocumentReferenceCufe = $resultRef['cufe'];
	$IDInvoiceDocumentReferenceIssueDate = $resultRef['datef'];
	
	// HALLA EL TaxExclusiveAmount
	$cadenaTaxExclusiveAmount = "
		SELECT TRUNCATE(SUM((total_ht * -1)), 2) AS TaxExclusiveAmount
	...
	if($resultTaxExclusiveAmount['TaxExclusiveAmount'] == NULL) {
		...
		$Percent1="19.00";
		//$Percent1 = round((($row['tva']*100)/$TaxExclusiveAmount),2).'.00';
	}
	...
	if($validaciones) {
		...
		$curl.= '","ReceptorAddressCountrySubentity":"'.$row['ReceptorAddressCountrySubentity'];
		$curl.= '","ReceptorAddressCountrySubentityCode":"'.$row['ReceptorAddressCountrySubentityCode'];
		...
		CASE
			WHEN det.total_ht < 0 THEN TRUNCATE((det.total_ht * -1), 2)
			ELSE TRUNCATE(det.total_ht, 2)
		END AS LineExtensionAmount,	
		CASE
			WHEN det.total_tva < 0 THEN TRUNCATE((det.total_tva * -1), 2)
			ELSE TRUNCATE(det.total_tva, 2)
		END AS TaxAmount,
		'false' AS TaxEvidenceIndicator,
		CASE
			WHEN det.total_tva < 0 THEN TRUNCATE((det.total_ht * -1), 2)
			ELSE TRUNCATE(det.total_tva, 2)
		END AS TaxableAmount,
		TRUNCATE(det.tva_tx, 2) AS Percent,
		'01' AS TaxSchemeID,     
		CASE
			WHEN det.subprice < 0 THEN TRUNCATE((det.subprice * -1), 2)
			ELSE TRUNCATE(det.subprice, 2)
		END AS PriceAmount,	
		'EA' AS unitCode,
